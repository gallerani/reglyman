...
Preprocess=ProbData(0.95, 0, mu, sigma)
...
def perform_inversion(Probability_data, init):
    ...
    descent = Gradient_Descent(setting, Probability_data, np.log(init), stepsize=0.1, alpha=2, penalty=penalty)
    
    stoprule = (
    rules.CombineRules(
    (rules.CountIterations(300),
    rules.RelativeChangeData(setting.Hcodomain.norm, Probability_data, 0.005))))
